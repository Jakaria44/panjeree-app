...
export default function HomeScreen() {
  ...
  const handleTestAPI = async () => {
    try {
      await refetch("10");
      Alert.alert("Success", `Loaded ${subjects.length} subjects from API`);
    } catch (err) {
      Alert.alert("Error", "Failed to load subjects from API");
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      <ThemedView style={styles.container}>
        <ScrollView
          contentContainerStyle={{ paddingTop: insets.top > 0 ? 0 : 16 }}
        >
          <PageHeader
          ...
          <View style={styles.featuresContainer}>
            ...
            >
              <ThemedText style={styles.testButtonText}>
                {loading ? "Loading..." : "Test API Integration"}
              </ThemedText>
            </TouchableOpacity>
            ...
            {subjects.length > 0 && (
              <ThemedText style={styles.successText}>
                Loaded {subjects.length} subjects successfully
              </ThemedText>
            )}
          </View>
        </ScrollView>
